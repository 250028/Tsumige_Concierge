...
| 完了 | 残り |
|---|---|
| ✅ DB設計・schema.sql | ❌ 認証（登録・ログイン） |
| ✅ Prisma セットアップ | ❌ 積みゲーリスト一覧 |
| ✅ マイグレーション | ❌ ゲーム登録 |
| ✅ シードデータ | ❌ ゲーム詳細 |
| ✅ lib/prisma.ts | ❌ 画面の実装 |
...
```
新規登録ページ /login → API Route → DB保存
ログインページ /login → API Route → iron-session でセッション発行
...
```
一覧 /list       → games テーブルを SELECT
登録 /games/new  → games テーブルに INSERT
詳細 /games/[id] → games テーブルを SELECT（1件）
```
